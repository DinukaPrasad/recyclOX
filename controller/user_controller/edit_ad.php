...
// Validate input
if (!$ad_id || !$category || !$location || !$weight || !$price || !$status || !$description) {
...
if ($ad_image && $ad_image['error'] === UPLOAD_ERR_OK) {
    ...
    $file_name = basename($ad_image['name']);
...
$sql = "UPDATE Advertisements 
        SET category_id = (SELECT category_id FROM GarbageCategory WHERE category_name = ?),
            city_id = (SELECT id FROM cities WHERE name_en = ?),
            weight = ?, 
            price = ?, 
            status = ?, 
            description = ?" .
...
if ($image_path) {
    $stmt->bind_param("ssdssssi", $category, $location, $weight, $price, $status, $description, $image_path, $ad_id, $_SESSION['user_id']);
} else {
    $stmt->bind_param("ssdsssi", $category, $location, $weight, $price, $status, $description, $ad_id, $_SESSION['user_id']);
}
